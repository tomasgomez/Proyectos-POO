...
#define RIGHT  4
   ...
   CVector Ball::getAceleration(){
       CVector aceleration;
       for(Spring * spring: joinedSprings)
         aceleration += spring->getForce(this)/mass;
       return aceleration ;
   }
   ...
   // you need to review, complete and/or change this code
     ...
     accel_tPlusDelta = getAceleration();
     speed_tPlusDelta = speed + (accel_tPlusDelta*(3.0)-(accel_t))*(delta_t/2);
     pos_tPlusDelta = pos_t + speed*delta_t + (accel_tPlusDelta*(4.0)-(accel_t))*((delta_t*delta_t)/6);
   ...
   void Ball::setSpring(Spring * spring){
   	  joinedSprings.push_back(spring);      
   }